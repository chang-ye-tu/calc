...
\begin{prp}[定積分基本性質]
  ...
  \begin{enumerate}\setlength\itemsep{0em}
    ...
    \item 若 $f(x)$ 為奇函數 $\ds\int_{-a}^a f(x)\,\mathrm{d}x = 0$
    \item 若 $f(x)$ 為偶函數 $\ds\int_{-a}^a f(x)\,\mathrm{d}x = 2\int_0^a f(x)\,\mathrm{d}x$
  \end{enumerate}
\end{prp}
...
\begin{ex}
  \begin{enumerate}\setlength\itemsep{0em}
    ...
    \item $\ds\int_{-a}^a |x|\,\mathrm{d}x = 2\int_0^a |x|\,\mathrm{d}x = a^2$（$|x|$ 為偶函數；兩 $a\times a$ 直角三角形面積）
    ...
    \item $\ds\int_0^2 \sqrt{4 - x^2}\cdot\sgn(1 - x)\,\mathrm{d}x = \sqrt{3} - \frac{\pi}{3}$ 
