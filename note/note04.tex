...
\begin{exe} 以部份積分法求下列不定積分。注意：可能會因為常數項而跟此處答案不同。
  \begin{enumerate}\setlength\itemsep{0em}
    ...
    \item $\ds\int\frac{\ln x}{\sqrt{1 + x}}\,\mathrm{d}x {\color{C2}\;= \ln x\cdot 2\sqrt{1 + x} - 4\sqrt{1+x} - 2\ln\Big(\sqrt{1 + x}-1\Big) + 2\ln\Big(\sqrt{1+x} + 1\Big) + c}$
  \end{enumerate}
\end{exe}
...
\begin{thm}[微積分基本定理（Fundamental Theorem of Calculus，FTC）]
  \begin{enumerate}\setlength\itemsep{0em}
    ...
    \item 若 $\ds G'(x) = f(x)\;\forall\,x\in[a, b]$，則 $\ds\int_a^b f(x)\,\mathrm{d}x = G(b) - G(a)$。
  \end{enumerate}
\end{thm}
\begin{rmk}
