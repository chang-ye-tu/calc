...
\begin{ex}
  求 $\ds\int e^{ax}\cos bx\,\mathrm{d}x$ 與 $\ds\int e^{ax}\sin bx\,\mathrm{d}x$。
\end{ex}
...
\begin{sol} 
  ...
  \hspace{5mm}
  \begin{minipage}{0.6\textwidth}
    $\ds\int e^{ax}\cos bx\,\mathrm{d}x = \frac{1}{a}e^{ax}\sin bx - \frac{b}{a^2}e^{ax}\cos bx - \frac{b^2}{a^2}\int e^{ax}\sin bx\,\mathrm{d}x\\ \ie\int e^{ax}\sin bx\,\mathrm{d}x = \frac{e^{ax}(a\sin bx - b\cos bx)}{a^2 + b^2}$
  \end{minipage}
\end{sol}
...
\begin{exe} 以部份積分法求下列不定積分。注意：可能會因為常數項而跟此處答案不同。
  \begin{enumerate}\setlength\itemsep{0em}
    ...
    \item $\ds\int x e^{\sqrt{x}}\,\mathrm{d}x {\color{C2}\;= e^{\sqrt{x}}\big(2x\sqrt{x}-6x + 12\sqrt{x} - 12\big) + c}$
...
%\begin{ex}
%  求 $\ds\int\frac{\ln x}{\sqrt{1 + x}}\,\mathrm{d}x$。
%\end{ex}
%
%\begin{sol}
%  \begin{itemize}\setlength\itemsep{0em}
%    \item[]
%    \item 令 $\ds u = \ln x$，則 $\ds\text{d}u = \frac{1}{x}\,\text{d}x$；$\ds\text{d}v = \frac{1}{\sqrt{1 + x}}\,\text{d}x$，則 $\ds v = 2\sqrt{1 + x}$。 故
%    \begin{align}\label{1}
%      \int\frac{\ln x}{\sqrt{1 + x}}\,\text{d}x = \ln x\cdot 2\sqrt{1 + x} - 2 \int\frac{\sqrt{1 + x}}{x}\,\text{d}x
%    \end{align}
%    \item 求 $\ds\int\frac{\sqrt{1 + x}}{x}\,\text{d}x$，令 $\ds w = \sqrt{1 + x}\ie w^2 = 1 + x \ie x= w^2 - 1$，$\ds 2w\,\text{d}w = \text{d}x$，故
%    \begin{align}\label{2}
%      \int\frac{\sqrt{1 + x}}{x}\,\text{d}x &= \int\frac{w}{w^2 - 1}\cdot 2w\,\text{d}w = 2\int\frac{w^2}{w^2 - 1}\,\text{d}w = 2\int\frac{(w^2 - 1) + 1}{w^2 - 1}\,\text{d}w \nonumber \\
%      &= 2 w + 2\int\frac{1}{w^2 - 1}\,\text{d}w = 2\sqrt{1 + x} + 2\int\frac{1}{w^2 -1}\,\text{d}w
%    \end{align} 
%    \item 求 $\ds\int\frac{1}{w^2 - 1}\,\text{d}w$，由部份分式 $\ds\frac{1}{w^2 - 1} = \frac{1}{2}\,\Big(\frac{1}{w - 1} - \frac{1}{w + 1}\Big)$
%    \begin{align}\label{3}
%      \int\frac{1}{w^2 - 1}\,\text{d}w &= \frac{1}{2}\int\Big(\frac{1}{w - 1} - \frac{1}{w + 1}\Big)\,\text{d}w \nonumber \\ 
%      &=\frac{1}{2}\,\big(\ln(w - 1) - \ln(w + 1)\big) = \frac{1}{2}\,\big(\ln(\sqrt{1 + x} - 1) - \ln(\sqrt{1 + x} + 1)\big)
%    \end{align} 
%    \item 由 \eqref{1}、\eqref{2}、\eqref{3} 得 $\ds\int\frac{\ln x}{\sqrt{1 + x}}\,\mathrm{d}x {\;= \ln x\cdot 2\sqrt{1 + x} - 4\sqrt{1+x} - 2\ln(\sqrt{1 + x}-1) + 2\ln(\sqrt{1+x} + 1) + c}$
%  \end{itemize}
%\end{sol}
