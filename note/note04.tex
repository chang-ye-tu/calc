...
%\begin{align*}
%S_1=\big\{\ (x,y)\ \big|\ a\leqslant x\leqslant b\,,\, g(x)\leqslant y\leqslant f(x)\ \big\}
%\end{align*}
%that is sketched in the left hand figure below.
%\begin{figure}
% \centering
% \includegraphics[width=.95\textwidth]{area_between2}
%\end{figure}
%We already know that $\int_a^b f(x)\,\text{d}x$ is the area of the region
%\begin{align*}
%S_2=\big\{\ (x,y)\ \big|\ a\leqslant x\leqslant b\,,\, 0\leqslant y\leqslant f(x)\ \big\}
%\end{align*}
%sketched in the middle figure above and that $\int_a^b g(x)\,\text{d}x$ is the area of the
%region
%\begin{align*}
%S_3=\big\{\ (x,y)\ \big|\ a\leqslant x\leqslant b\,,\, 0\leqslant y\leqslant g(x)\ \big\}
%\end{align*}
...
%\begin{align*}
%  \int_a^b f(x)\,\text{d}x -  \int_a^b g(x)\,\text{d}x
%  &=  \int_a^b \big(f(x)-g(x)\big)\,\text{d}x
%\end{align*}
...
%lies above the line $y=x$ for all $-1\leqslant x\leqslant 1$.
%
%\item We are to find the area of the shaded region. Each point $(x,y)$ in this
%shaded region has $-1\leqslant x\leqslant 1$ and $x \leqslant y \leqslant 4-x^2$. When we were
%defining the integral (way back in Definition we used $a$ and $b$
...
%\begin{align*}
%\big\{\ (x,y)\ \big|\ a\leqslant x\leqslant b,\ B(x)\leqslant y\leqslant T(x)\ \big\}
%\end{align*}
...
%\item subdivide the region into $n$ narrow slices, each of width $\Delta x=\frac{b-a}{n}$.
...
%\item Thus the area of slice $i$ is approximately $\big[T(x_i^*)-B(x_i^*)\big]\Delta x$ (as
...
%\begin{align*}
%  \text{Area} &\approx \sum_{i=1}^n  \big[T(x_i^*)-B(x_i^*)\big]\Delta x
%\end{align*}
%\item By taking the limit as $n \to \infty$ (i.e. taking the limit as the width of the
%rectangles goes to zero), we convert the Riemann sum into a definite integral
% and at the same time our approximation of the area
%becomes the exact area:
%\begin{align*}
%\lim_{n\rightarrow\infty}\sum_{i=1}^n  \big[T(x_i^*)-B(x_i^*)\big]\Delta x
%&=\int_a^b \big[T(x)-B(x)\big]\text{d}x & \text{Riemann sum $\to$ integral}\\
%&=\int_{-1}^1\big[(4-x^2)-x\big]\text{d}x\\
%&=\int_{-1}^1\big[4-x-x^2\big]\text{d}x \\
...
%\end{itemize}
%\begin{align}
%  \text{Area} = \int_a^b \big[T(x)-B(x)\big]\text{d}x.
%\end{align}
%\end{sol}
%
%\begin{ex}           
...
%\includegraphics[height=5cm]{area_between3}
%\end{figure}
%and verify that $y=T(x)=4-x^2$ lies above the curve $y=B(x)=x$ on the region $-1\leq x\leq 1$.
%
...
%\begin{align*}
%  \text{Area} &=\int_a^b \big[T(x)-B(x)\big]\text{d}x \\
%&=\int_0^2\big[(6x-2x^2)-(x^2)\big]\text{d}x\\
%&=\int_0^2\big[6x-3x^2\big]\text{d}x \\
...
%vertical rectangles, each of width $\Delta x$. Two of those rectangles are illustrated in
%the sketch
%\begin{figure}
%\centering
...
%\begin{align*}
%B(x)&=\begin{cases}
%           -\sqrt{2x+6} & \text{if } -3\leqslant x\leqslant -1 \\
%           x-1 & \text{if }-1\leqslant x\leqslant 5
%      \end{cases}
%\end{align*}
%\item Just as before, the area is still given by the formula $\int_a^b
%\big[T(x)-B(x)\big]\text{d}x$, but to accommodate our $B(x)$, we have to split
%up the domain of integration when we evaluate the integral.
%\begin{align*}
%\int_a^b \big[T(x)-B(x)\big]\text{d}x
%  &=  \int_{-3}^{-1} \big[T(x)-B(x)\big]\text{d}x
%          +\int_{-1}^5 \big[T(x)-B(x)\big]\text{d}x \\
%  &= \int_{-3}^{-1} \big[\sqrt{2x+6}-(-\sqrt{2x+6})\big]\text{d}x
%          +\int_{-1}^5 \big[\sqrt{2x+6}-(x-1)\big]\text{d}x \\
%  &= 2\int_{-3}^{-1} \sqrt{2x+6}\text{d}x
%          +\int_{-1}^5 \sqrt{2x+6} - \int_{-1}^5(x-1)\text{d}x
%\end{align*}
%\item The third integral is straightforward, while we evaluate the first two via the
%substitution rule. In particular, set $u=2x+6$ and replace $\text{d}x \rightarrow
%\frac{1}{2}\text{d}u$. Also $u(-3)=0, u(-1)=4, u(5)=16$. Hence
%\begin{align*}
%\text{Area}
%  &= 2\int_0^4\sqrt{u}\ \frac{\text{d}u}{2}
%          +\int_4^{16} \sqrt{u}\ \frac{\text{d}u}{2}
%          -\int_{-1}^5 (x-1)\text{d}x \\
...
%region has $-2\leqslant y\leqslant 4$ and $\frac{1}{2}(y^2-6)\leqslant x \leqslant y+1$.
%\item Let's use
%\begin{itemize}
%\item
...
%and the shaded region is
%\begin{align*}
%\big\{\ (x,y)\ \big|\ c\leqslant y\leqslant d,\ L(y)\leqslant x\leqslant R(y)\ \big\}
%\end{align*}
%\item Our strategy is now nearly the same as that used in:
%\begin{itemize}
% \item Pick a natural number $n$ (that we will later send to infinity), then
% \item subdivide the interval $c\leqslant y\leqslant d$ into $n$ narrow subintervals, each of width
%$\Delta y=\frac{d-c}{n}$. Each subinterval cuts a thin horizontal slice from the region (see
%the figure below).
...
%is at $x=R(y_i^*)$.
%\item Thus the area of slice $i$ is approximately $\big[R(x_i^*)-L(x_i^*)\big]\Delta y$.
%\end{itemize}
%\begin{figure}
%\centering
...
%\begin{align*}
%\lim_{n\rightarrow\infty}\sum_{i=1}^n \big[R(y_i^*)-L(y_i^*)\big]\Delta y
%&=\int_c^d \big[R(y)-L(y)\big]\text{d}y & \text{Riemann sum $\rightarrow$ integral}\\
%&=\int_{-2}^4 \big[(y+1)-\tfrac{1}{2}\big(y^2-6\big)\big]\text{d}y \\
%&=\int_{-2}^4 \big[-\tfrac{1}{2}y^2+y+4\big]\text{d}y \\
...
%
%\begin{ex}
%  Find the area between the curves $y=\dfrac{1}{\sqrt{2}}$ and $y=\sin(x)$ with $x$ running from $0$ to $\nicefrac{\pi}{2}$.
%\end{ex}
%
%\begin{sol}
%  \begin{itemize}
...
%          \nicefrac{1}{\sqrt{2}} & \text{if $0\leqslant x\leqslant \nicefrac{\pi}{4}$}\\
%          \sin(x)& \text{if $\nicefrac{\pi}{4}\leqslant x\leqslant \nicefrac{\pi}{2}$}
%      \end{cases}\right\}\qquad
%B(x)=\left.\begin{cases}
%          \sin(x) & \text{if $0\leqslant x\leqslant \nicefrac{\pi}{4}$}\\
...
%we must split the domain of the integral in two at that point.
%
%\item Our integral over the domain $0\leq x \leq \nicefrac{\pi}{2}$ is split into an
%integral over $0\leqslant x\leqslant \nicefrac{\pi}{4}$ and one over $\nicefrac{\pi}{4}\leqslant x\le
%\nicefrac{\pi}{2}$:
%\begin{align*}
%\text{Area} &= \int_0^{\nicefrac{\pi}{2}} \big[T(x)-B(x)\big]\text{d}x \\
%   &= \int_0^{\nicefrac{\pi}{4}} \big[T(x)-B(x)\big]\text{d}x
%      +\int_{\nicefrac{\pi}{4}}^{\nicefrac{\pi}{2}} \big[T(x)-B(x)\big]\text{d}x \\
%   &= \int_0^{\nicefrac{\pi}{4}} \Big[\frac{1}{\sqrt{2}}-\sin(x)\Big]\text{d}x
%      +\int_{\nicefrac{\pi}{4}}^{\nicefrac{\pi}{2}}
%                       \Big[\sin(x)-\frac{1}{\sqrt{2}}\Big]\text{d}x \\[0.1in]
...
%pancake is approximately a squat cylinder of height $\Delta x=\nicefrac{h}{n}$. This
...
%\end{figure}
%
%\item We follow the method we used in, except that
%our slices are now pancakes instead of rectangles.
%\begin{itemize}
% \item Pick a natural number $n$ (that we will later send to infinity), then
%\item subdivide the cone into $n$ thin pancakes, each of width $\Delta x=\frac{h}{n}$.
%\item For each $i=1,2,\dots,n$, pancake number $i$ runs from
%$x=x_{i-1}=(i-1)\cdot\Delta x$ to $x=x_i=i\cdot\Delta x$, and we approximate its
%volume by the volume of a squat cone. We pick a number $x_i^*$ between $x_{i-1}$ and
%$x_i$
%and approximate the pancake by a cylinder of height $\Delta x$ and radius $\frac{x_i^*}{h}r$.
%  \item Thus the volume of pancake $i$ is
%approximately $\pi \left( \frac{x_i^*}{h}r\right)^2 \Delta x$ (as shown in the figure above).
%\end{itemize}
%
...
%the pancakes goes to zero), we convert the Riemann sum into a definite integral and at the same time our approximation of the volume
%becomes the exact volume:
%\begin{align*}
%\int_0^h \pi \Big(\frac{x}{h}r\Big)^2\text{d}x
%\end{align*}
%\end{itemize}
%\begin{itemize}
...
%  \includegraphics{cone}
%\end{figure}
%and think of slicing it into thin pancakes, each of width $\text{d}x$.
%\begin{figure}
%\centering
...
%\item thickness $\text{d}x$ --- we have done something a little sneaky here, see the
%discussion below.
%\item volume $\pi \big(\frac{x}{h}r\big)^2\text{d}x$
%\end{itemize}
...
%slice the cone into $n$ pancakes each of thickness $\Delta x = \nicefrac{h}{n}$ and then
%take the limit as $n \to \infty$. This led to the Riemann sum
%\begin{align*}
%  \sum_{i=1}^n \pi \left( \frac{x_i^*}{h} r \right)^2 \Delta x && \text{which becomes}
%  \int_0^h \pi \left( \frac{x}{h} r \right)^2 \text{d}x
%\end{align*}
%So knowing that we will replace
%\begin{align*}
%  \sum_{i=1}^n &\longrightarrow \int_0^h\\
%  x_i^* &\longrightarrow x\\
%  \Delta x &\longrightarrow \text{d}x
%\end{align*}
...
%is rotated around the line $y=2$. Find the volume of the region swept out.
%\end{ex}
%
%
%\begin{figure}
...
%eye. Here is what you see as the rotation takes place.
%\begin{figure}
%\centering
...
%\begin{sol}
%\begin{itemize}
% \item Consider the region and cut it into thin vertical strips of width $\text{d}x$.
%\begin{figure}
% \centering
...
% \item Now slice the region into thin horizontal strips of width $\text{d}y$.
%\begin{figure}
% \centering
...
%\item thickness is $\text{d}y$ and hence
%\item has volume $\pi(r_{out}^2 - r_{in}^2)\text{d}y = \pi(16-y^4)\text{d}y$.
%\end{itemize}
%\item As our bottommost strip is at $y=0$ and our topmost
%strip is at $y=2$, the total
%\begin{align*}
%\text{Volume}
%&= \int _0^2 \pi(16-y^4) \text{d}y
...
%\item So the \emph{full} pancake at height $z$ is a square of side $\frac{h-z}{h}b$. As a
...
%thickness $\text{d}z$ and hence
%\item volume $\big(\frac{h-z}{h}b\big)^2\text{d}z$.
%\end{itemize}
%\item The volume of the whole pyramid (not just the part of the pyramid in the first
%octant) is
%\begin{align*}
%\int_0^h \Big(\frac{h-z}{h}b\Big)^2\text{d}z
%&=\frac{b^2}{h^2} \int_0^h (h-z)^2\text{d}z \\
%&=\frac{b^2}{h^2} \int_h^0 -t^2\text{d}t & \text{substitution rule with
%$t=(h-z), \text{d}z\to-\text{d}t$} \\
...
%\begin{align*}
%V&=\int_0^{20}2x\sqrt{20^2-x^2}\text{d}x
%\intertext{Make the change of variables $u=20^2-x^2$ (don't forget to change $\text{d}x
%\rightarrow -\frac{1}{2x}\text{d}u$):}
%V&=\int_{20^2}^{0}-\sqrt{u}\,\text{d}u\\
...
%\begin{align*}
%V&=\half\int_{-20}^{20}(20^2-y^2)\text{d}y\\
%&=\int_0^{20} (20^2-y^2)\text{d}y\\
...
%\item thickness is $\text{d}x$ and hence
