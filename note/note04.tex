...
\begin{exe} 以部份積分法求下列不定積分. 注意: 可能會因為常數項而跟此處答案不同. 
  \begin{enumerate}\setlength{\itemsep}{0pt}
    \item $\ds\frac{\sin^{-1}x}{x^2}\,\mathrm{d}x {\color{C2}\;= - \frac{\sin^{-1}x}{x} - \ln\Big|\frac{1 - \sqrt{1 - x^2}}{x}\Big| + c}$
    \item $\ds\ln(x + \sqrt{1 + x^2})\,\mathrm{d}x {\color{C2}\;= x\,\ln(x + \sqrt{1 + x^2}) - \sqrt{1 + x^2} + c}$
