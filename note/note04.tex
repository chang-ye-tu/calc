...
\begin{fact}
  \begin{enumerate}\setlength\itemsep{0em}
    ...
    \item 依序左上連右下斜線函數相乘，符號正負相間；最下列直線寫成兩邊函數相乘的積分
...
\subsubsection*{遞迴式}
\begin{itemize}
  \item 定義積分 $\ds I_n = \int\frac{1}{(x^2 + a^2)^n}\,\mathrm{d}x$, $n\in\mathbb{N}$, $n\geqslant 1$，則 $\ds I_n = \int\frac{1}{(x^2 + a^2)^n}\,\mathrm{d}x = \frac{1}{a^2}\int\frac{(x^2 + a^2) - x^2}{(x^2 + a^2)^n}\,\mathrm{d}x = \frac{1}{a^2}\int\frac{1}{(x^2 + a^2)^{n - 1}}\,\mathrm{d}x - \frac{1}{a^2}\int\frac{x^2}{(x^2 + a^2)^n}\,\mathrm{d}x$
  \item 考慮 $\ds\frac{1}{a^2}\int\frac{x^2}{(x^2 + a^2)^n}\,\mathrm{d}x$，令 $u = x$，則 $\ds\mathrm{d}u = \mathrm{d}x$。令 $\ds\text{d}v = \frac{x}{(x^2 + a^2)^n}\,\mathrm{d}x$，則 $\ds v = -\frac{1}{2(n-1)}\frac{1}{(x^2 + a^2)^{n-1}}$，故 $\ds\int\frac{x^2}{(x^2 + a^2)^n}\,\mathrm{d}x = -\frac{1}{2(n-1)}\frac{x}{(x^2 + a^2)^{n-1}} + \frac{1}{2(n-1)}\int\frac{1}{(x^2 + a^2)^{n-1}}\,\mathrm{d}x =-\frac{1}{2(n-1)}\frac{x}{(x^2 + a^2)^{n-1}} + \frac{1}{2(n-1)}I_{n-1}$
  \item 所以 $\ds I_n = \frac{1}{a^2}\int\frac{1}{(x^2 + a^2)^{n - 1}}\,\mathrm{d}x - \frac{1}{a^2}\int\frac{x^2}{(x^2 + a^2)^n}\,\mathrm{d}x = \frac{1}{a^2}\,I_{n-1} - \frac{1}{a^2}\bigg(-\frac{1}{2(n-1)}\frac{x}{(x^2 + a^2)^{n-1}} + \frac{1}{2(n-1)}I_{n-1}\bigg) = \frac{1}{a^2}\frac{2n-3}{2(n-1)}\,I_{n-1} + \frac{1}{a^2}\frac{1}{2(n-1)}\frac{x}{(x^2 + a^2)^{n-1}}$
  \item 由此遞迴關係及 $\ds I_1 = \frac{1}{a}\tan^{-1}\frac{x}{a}$，可得 $I_n$, $n\in\mathbb{N}$, $n > 1$。例：令 $n=2$，則 $\ds I_2 = \frac{1}{2a^2}\,I_1 + \frac{x}{2a^2(x^2+a^2)} = \frac{1}{2a^3}\tan^{-1}\frac{x}{a} + \frac{x}{2a^2(x^2+a^2)}$。
\end{itemize}
