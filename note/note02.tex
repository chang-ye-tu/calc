...
\begin{sol}
  \begin{enumerate}\setlength\itemsep{0em}
    \item[]
    \item 令 $\ds f(u) = u^{2024}$, $g(x) = x^3 - 1$, 則 $\ds f'(u) = 2024\,u^{2023}$, $\ds(x^3 - 1)^{2024} = f(g(x))$. \\由鏈鎖律 $\ds((x^3 - 1)^{2024})' = (f(g(x)))' = f'(g(x))\cdot g'(x) = 2024\cdot(x^3 - 1)^{2023}\cdot(3x^2)$. 
...
\begin{prf}
  \begin{minipage}{0.52\textwidth}
    \includegraphics[scale=.85,page=1]{fig/sind.pdf}
    \includegraphics[scale=.6,page=2]{fig/sind.pdf}
  \end{minipage}
  \begin{minipage}{0.48\textwidth}
    $\ds\text{由面積關係}\;\Delta OPR \leqslant \text{扇形}\;OPR \leqslant \Delta OQR \\ \ie\sin h\leqslant h \leqslant\frac{\sin h}{\cos h} \ie\cos h\leqslant\frac{\sin h}{h}\leqslant 1$. \\ 由 $\ds\lim_{h\to 0}\cos h = 1$ 與夾擊定理, $\ds\lim_{h\to 0}\frac{\sin h}{h} = 1$. 
  \end{minipage}
\end{prf}
...
\begin{sol}
  \begin{itemize}\setlength\itemsep{0em}
    \item[]
    \item $\ds f(x) = (\cos x)^{(\sin x)} = e^{\ln(\cos x)^{(\sin x)}} = e^{(\sin x)\,\ln(\cos x)} \ie f'(x) = e^{(\sin x)\,\ln(\cos x)}\cdot\big(\cos x\cdot\ln(\cos x) + \sin x\cdot\big(\frac{-\sin x}{\cos x}\big)\big) = (\cos x)^{(\sin x)}\cdot\big(\cos x\cdot\ln(\cos x) + \sin x\cdot\big(\frac{-\sin x}{\cos x}\big)\big)$ 
    \item 另解: $\ds\log f(x) = \sin x\cdot\ln(\cos x)\ie f'(x) = (\cos x)^{(\sin x)}\cdot\big(\cos x\cdot\ln(\cos x) + \sin x\cdot\big(\frac{-\sin x}{\cos x}\big)\big)$
  \end{itemize}
\end{sol}
...
\begin{exe} 求下列導函數.  
  ...
  \begin{multicols}{2}
    \begin{enumerate}\setlength{\itemsep}{0pt}
      ...
      \item $\ds\left(\sin x^3\right)' = {\color{C2}\;3x^2 \cos x^3}$
