...
\begin{thm}[Clairaut-Young-Schwarz]
...
\section*{5.4 鏈鎖律} 
\begin{thm}
  給定 $\ds F(s, t) = f\big(x(s,t),\,y(s,t)\big)$，則 \\
  $\ds\pdiff{F}{s}(s,t) = \pdiff{f}{x}\big(x(s,t),\,y(s,t)\big)\,\pdiff{x}{s}(s,t) + \pdiff{f}{y}\big(x(s,t),\,y(s,t)\big)\,\pdiff{y}{s}(s,t)$ \\ 
  $\ds\pdiff{F}{t}(s,t) = \pdiff{f}{x}\big(x(s,t),\,y(s,t)\big)\,\pdiff{x}{t}(s,t) + \pdiff{f}{y}\big(x(s,t),\,y(s,t)\big)\,\pdiff{y}{t}(s,t)$
\end{thm}
$\ds\diff{}{t}f\big(x(t)\big) = \diff{f}{x}\big(x(t)\big)\,\diff{x}{t}(t)$

\begin{ex}
  若 $\ds f(x, y) = e^{xy}$，$\ds x(s, t) = s$，$\ds y(s, t) = \cos t$；定義 $\ds F(s, t) = f\big(x(s,t),\,y(s,t)\big)$，求 $\ds\pdiff{F}{s}(s,t)$。
\end{ex}
\begin{sol}
  由 $\ds\pdiff{F}{s}(s,t) = \pdiff{f}{x}\big(x(s, t),\,y(s,t)\big)\,\pdiff{x}{s}(s,t) + \pdiff{f}{y}\big(x(s,t),\,y(s,t)\big)\,\pdiff{y}{s}(s,t)$，$\ds\pdiff{f}{x}(x,y) = ye^{xy}$ $\pdiff{f}{x}\big(x(s,t), y(s,t)\big) = y(s,t)e^{x(s,t)\,y(s,t)} = \cos t e^{s\cos t}$ $\ds\pdiff{f}{y}(x,y) = xe^{xy}$ $\pdiff{f}{y}\big(x(s,t),y(s,t)\big) = x(s,t)\,e^{x(s,t)\,y(s,t)} = s e^{s\cos t}$ $\ds\pdiff{x}{s} = 1$$\ds\pdiff{y}{s} = 0$
$\ds\pdiff{F}{s}(s,t)=\overbrace{\left\{\cos t e^{s\cos t}\right\}}^{\pdiff{f}{x}}\overbrace{(1)}^{\pdiff{x}{s}} + \overbrace{\left\{s\ e^{s\cos t}\right\}}^{\pdiff{f}{y}}\overbrace{(0)}^{\pdiff{y}{s}} = \cos t\ e^{s\cos t}$
\end{sol}
...
$$
\end{ex}
\begin{ex}[$\frac{\partial^2\hfill}{\partial t^2}f(x+ct)$]
