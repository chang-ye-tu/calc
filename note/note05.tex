...
\begin{sol}
  局域下 $z$ 為 $x$、$y$ 之函數；$\ds x\sin z - z^2 y = 1$ 對 $x$ 偏微分得 $\ds\sin z + x\cos z\,\pdiff{z}{x} - 2yz\,\pdiff{z}{x} = 0 \ie \pdiff{z}{x} = \frac{x}{z}$；對 $y$ 偏微分得 $\ds 2y + 2z\,\pdiff{z}{y} = 0 \ie \pdiff{z}{x} = -\frac{y}{z}$。故 $\ds x\,\pdiff{z}{x} + y\,\pdiff{z}{y} = -\frac{x^2 + y^2}{z} = \frac{z^2 - 1}{z} = z - \frac{1}{z}$。
\end{sol}
...
\begin{dfn}
  令 $S\subseteq\mathbb{R}^n$，$\vc\in S$，$\vu\in\mathbb{R}^n$，可微函數 $\ds f:S\to\mathbb{R}$；$\mathbb{R}^n$ 中直角座標系單位向量為 $\ve_1,\,\ve_2,\,\ldots,\,\ve_n$。
  \begin{itemize}\setlength{\itemsep}{0pt}
    \item $f$ 在 $\vc$ 之 $\ds\vu$ 方向導數 (directional derivative) 為 $\ds D_{\vu}f(\vc) = \lim_{h\to 0}\frac{f(\vc + h\vu) - f(\vc)}{h}$。
...
\begin{dfn}
  給定 $\ds S\subseteq\mathbb{R}^n$，$\ds f:S\to\mathbb{R}$，$\ds B(\vx, h)\equiv\{\vy\in\mathbb{R}^n\;|\;|\vy - \vx| < h\}$。
...
\begin{thm}[極值必要條件]
  給定 $\ds S\subseteq\mathbb{R}^n$ 與可微函數 $\ds f:S\to\mathbb{R}$，若 $f$ 在 $S$ 之內點 $\vc$ 有極值，則 $\ds\nabla f(\vc) = \vZero$。
\end{thm}

\begin{prf}
  若 $\ds\vc = (c_1,\,c_2,\,\ldots,\,c_n)$，令 $\ds g_j(t)\equiv f(c_1,\,c_2,\,\ldots,\,c_{j - 1},\,t,\,c_{j + 1},\,\ldots,\,c_n)$，$j = 1,\,2,\,\ldots,\,n$。因 $f$ 在 $\vc$ 有極值 $f(\vc) = g_j(c_j)$，$g_j$ 在 $c_j$ 有極值 $\ie g'(t)\,\big|_{t = c_j} = 0 \ie f_j(\vc) = 0\;\forall\,j$，故 $\ds\nabla f(\vc) = \vZero$。 
\end{prf}

\begin{fact}
  給定 $\ds S\subseteq\mathbb{R}^n$，若 $f:S\to\mathbb{R}$ 在 $\vc\in S$ 有極值，則 $\vc$ 為以下三情形之一：
  \begin{multicols}{2}
    \begin{itemize}\setlength\itemsep{0em}
      \item 臨界點 (critical point) ：$\ds \nabla f(\vc) = \vZero$。
      \item 奇異點 (singular point) ：$f$ 在 $\vc$ 不可微。 
      \item $S$ 的邊界點 (boundary)。
    \end{itemize}
  \end{multicols}
\end{fact}

\begin{dfn}[Hessian 矩陣]
  給定 $\ds S\subseteq\mathbb{R}^n$、$S$ 之內點 $\vc$、與可微函數 $\ds f:S\to\mathbb{R}$，
  \begin{align*}
    \vH(f, \vc) = \begin{pmatrix}f_{11} & f_{12} & \cdots & f_{1n} \\ f_{21} & f_{22} & \cdots & f_{2n} \\ \vdots & \vdots & \cdots & \vdots \\ f_{n1} & f_{n2} & \cdots & f_{nn}\end{pmatrix}, \qquad f_{ij} = \frac{\partial^2 f}{\partial x_j\partial x_i}(\vc), \qquad i,\,j = 1,\,2,\,\ldots,\,n.
  \end{align*}
\end{dfn}

\begin{dfn}[矩陣正 / 負定性] 
  給定 $n\times n$ 實矩陣 $\vM$。對任意 $\ds\vv\in\mathbb{R}^n\ne\vZero$，若 $\ds\vv\vM\vv^\top > 0$ 恆成立，則稱 $\vM$ 為正定 (positive-definite)；若 $\ds\vv\vM\vv^\top < 0$ 恆成立，則稱 $\vM$ 為負定 (negative-definite)。
\end{dfn}

\begin{thm}[二階導數判定法]
  給定 $\ds S\subseteq\mathbb{R}^n$ 與可微函數 $\ds f:S\to\mathbb{R}$，且 $f$ 在 $S$ 之內點 $\vc$ 有 $\ds\nabla f(\vc) = 0$。
  \begin{itemize}\setlength\itemsep{0em}
    \item 若 $\vH(f, \vc)$ 為正定，則 $f$ 在 $\vc$ 有極小值。
    \item 若 $\vH(f, \vc)$ 為負定，則 $f$ 在 $\vc$ 有極大值。
  \end{itemize}
\end{thm}

\begin{fact}
  給定 $\ds S\subseteq\mathbb{R}^2$ 與可微函數 $\ds f:S\to\mathbb{R}$，且 $f$ 在 $S$ 之內點 $(a, b)$ 有 $\ds\nabla f(a, b) = 0$。令 
  \begin{align*}
    D = f_{xx}(a, b)\cdot f_{yy}(a, b) - (f_{xy}(a, b))^2
  \end{align*}
  \begin{itemize}\setlength\itemsep{0em}
    \item 若 $D > 0$ 且 $\ds f_{xx}(a, b) > 0$，則 $f$ 在 $(a, b)$ 有極小值。
    \item 若 $D > 0$ 且 $\ds f_{xx}(a, b) < 0$，則 $f$ 在 $(a, b)$ 有極大值。
    \item 若 $D < 0$，則 $(a, b)$ 為鞍點 (saddle point)。
  \end{itemize}
\end{fact}
...
\begin{sol} 由 $\ds f_x(x, y) = 3x^2 + y^2 - 6x$，$\ds f_y(x, y) = 2xy - 8y$，臨界點為同時滿足以上二式為零之 $(x, y)$。故 
  \begin{align*}
    \big\{3x^2 + y^2 - 6x = 0\big\} \orr \big\{y\,(x - 4) = 0\big\} \ie \big\{y = 0,\;3x^2 - 6x = 0\big\}\orr\big\{x = 4,\;3\cdot 4^2 + y^2 + 6\cdot 4 = 0\big\}
  \end{align*}
  故臨界點為 $(0, 0)$，$(2, 0)$。又 $\ds f_{xx} = 6x - 6$，$\ds f_{yy} = 2x - 8$，$\ds f_{xy} = f_{yx} = 2y$，分類如下表：
  \begin{center}
...
\begin{sol}
  ...
  \end{align*}
  故臨界點為 $(0, 0)$，$(3, 0)$，$(0, 15)$，$(1,5)$。又 $\ds f_{xx} = 10 y$，$\ds f_{yy} = 2x$，$\ds f_{xy} = f_{yx} = 10x + 2y - 15$，分類如下表：
  \begin{center}
  \renewcommand{\arraystretch}{1.3}
  \begin{tabular}{cccc}
    \toprule
...
\begin{sol}
  ...
  \begin{itemize}\setlength\itemsep{0em}
    ...
    \item 邊界點 $x^2 + y^2 = 1$：令 $x = \cos t$，$y = \sin t$，$0\leqslant t\leqslant 2\pi$，則 $f(x, y)$ 變為 $\ds g(t)\equiv(\cos t + \sin t)\,e^{-1}$；$g'(t) = (-\sin t + \cos t)\,e^{-1} = 0$ 解得 $t = \frac{\pi}{4},\,\frac{5\pi}{4}$，亦即 $(x, y) = \big(\frac{1}{\sqrt{2}}, \frac{1}{\sqrt{2}}\big),\;\big(-\frac{1}{\sqrt{2}}, -\frac{1}{\sqrt{2}}\big)$。 
  \end{itemize}
  \begin{center}
  \renewcommand{\arraystretch}{1.3}
  \begin{tabular}{ccc}
    ...
    $\big(-\frac{1}{\sqrt{2}}, -\frac{1}{\sqrt{2}}\big)$ & $-\sqrt{2}\,e^{-1}$ &  \\ \bottomrule
  \end{tabular}
  \renewcommand{\arraystretch}{1.0}
  \end{center}
\end{sol}
...
\begin{thm}
  給定開集 $S\subseteq\mathbb{R}^n$，可微函數 $f:S\to\mathbb{R}$ 與 $g_j:S\to\mathbb{R}$，$j=1,\,2,\,\ldots,\,m$，$m < n$，及 $\ds X_0 = \big\{\vx\in S\;|\;g_j(\vx) = 0,\,j=1,\,2,\,\ldots,\,m\big\}$。若 $\ds\vx_0\in S\,\cap\,X_0$ 有極值且 $\det\big(D_i\,g_j(\vx_0)\big)\ne 0$，則 $\exists\,\lambda_1,\,\lambda_2,\,\ldots,\,\lambda_m$ 使得 $\ds D_i\,f(\vx_0) + \sum_{j = 1}^m\lambda_j\,D_i g_j(\vx_0) = 0$，$\ds i = 1,\,2,\,\ldots,\,n$。
\end{thm}
...
\begin{ex} 
  求 $\ds x^2 - 10 x - y^2$ 在限制式 $\ds x^2 + 4 y^2 = 16$ 下的最大值，最小值。
\end{ex}
...
\begin{sol}
  ...
  \end{align}
  由 \eqref{eq:2} $(1 - 4\lambda) y = 0$，則 $y = 0\orr\lambda = \frac{1}{4}$。若 $y = 0$，由 \eqref{eq:3} $x = \pm 4$；若 $\lambda=\frac{1}{4}$，由 \eqref{eq:1} $(1 + \lambda)\,x = 5\ie x = 4$，代入 \eqref{eq:3} 得 $y = 0$。故極值點為 $(x, y) = (4, 0),\,(-4, 0),\,(0, 4)$；$x^2 - 10x - y^2$ 最大值為 $56$ ($(x, y) = (-4, 0)$)，最小值為 $-24$ ($(x, y) = (4, 0)$)。
\end{sol}
