...
\begin{sol}
  由於 $\sin nx$ 為週期 $2\pi$ 之函數，只需驗證 $x\in(0, 2\pi)$。令 $\ds A_n(x) = \sum_{k = 1}^n\sin k x$，則 $\ds 2\sin\frac{x}{2}\cdot A_n(x) = \sum_{k = 1}^n 2\sin\frac{x}{2}\cdot\sin k x = \sum_{k = 1}^n\bigg(\cos\Big(k - \frac{1}{2}\Big)x - \cos\Big(k + \frac{1}{2}\Big)x\bigg) = \cos\frac{x}{2} - \cos\Big(n + \frac{1}{2}\Big)x$ $\ie$ $\ds|A_n(x)| = \Bigg|\frac{\cos\frac{x}{2} - \cos\Big(n + \frac{1}{2}\Big)x}{\sin\frac{x}{2}}\Bigg|\leqslant\frac{1}{\sin\frac{x}{2}}\;\forall\,x\in(0, 2\pi)$ 為有界，$\ds\lim_{n\to\infty}\frac{1}{n}\searrow 0$，由 Dirichlet 判定法 $\ds\sum_{n = 1}^\infty\frac{\sin nx}{n}$ 收斂，$\forall\,x\in\mathbb{R}$。
\end{sol}
