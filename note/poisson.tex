...
\end{enumerate}
令 $\ds P(k,\,T)$ 為在時間區間 $T$ 中發生 $k$ 次事件的機率. 由 (i) $\ds P(1,\,\Delta t) = \lambda\,\Delta t$, 由 (ii) $\ds P(k,\,\Delta t) = 0$, $k\geqslant 2$. 將 $T$ 分割成 $N$ 個短時間區段，亦即 $\ds T = N\Delta t$. 由 (iii) 「在不重疊之時間區間內, 事件各自發生的次數為獨立」可知 $\ds P(k,\,T) = \binom{N}{k}(\lambda\,\Delta t)^k(1 - \lambda\,\Delta t)^{N - k} = \frac{N(N - 1)\cdots(N - k + 1)}{k!}\,\frac{(\lambda\,T)^k}{N^k}\,\frac{\big(1 - \frac{\lambda\,T}{N}\big)^N}{\big(1 - \frac{\lambda\,T}{N}\big)^k}$. 固定 $k$, 當 $N\to\infty$ 時 $\ds\Big(1 + \frac{\alpha}{N}\Big)^N \to e^{\alpha} \ie P(k,\,T) \to \frac{(\lambda\,T)^k}{k!}e^{-\lambda\,T}$. 由此可知 Poisson 分配是二項分配 ${\sf B}(N,\,p,\,q)$ 的一種極限, 其中 $N\,p = \lambda\,T$, 然後 $N\to\infty$. 另外, 我們通常令 $\lambda\,T = m$; $m$ 表示在時間區間 $T$ 中的平均發生次數. 
...
\begin{ex}
  ...
  \begin{multicols}{2}
    \begin{enumerate}[label=(\alph*)]\setlength{\itemsep}{0pt}
      ...
      \item 令 $\ds P_X(k) = \frac{m^k}{k!}e^{-m}$, $k = 1,\,2,\,3,\,\cdots$. 求 $\expc\{X\}$ 與 $\var\{X\}$.
    \end{enumerate}
  \end{multicols}
\end{ex}
...
\begin{sol} 由 $e^x$ 之 MacLaurin 展開式 $\ds e^x = \sum_{k = 0}^{\infty}\frac{x^k}{k!}$, 代入 $x = \lambda\,T$ 得 $\ds e^{\lambda\,T} = \sum_{k = 0}^{\infty}\frac{(\lambda\,T)^k}{k!}$.
  \begin{enumerate}[label=(\alph*)]\setlength{\itemsep}{0pt}
    ...
    \item $\ds\expc\{X\} = \sum_{k = 0}^\infty k\frac{(\lambda\,T)^k}{k!}e^{-\lambda\,T} = e^{-\lambda\,T}\cdot\sum_{k = 1}^\infty k\frac{(\lambda\,T)^k}{k!} = e^{-\lambda\,T}\cdot\sum_{k = 0}^\infty\frac{(\lambda\,T)^{k + 1}}{k!} = \lambda\,T\cdot e^{-\lambda\,T}\cdot\sum_{k = 0}^\infty\frac{(\lambda\,T)^k}{k!} = \lambda\,T = m$.\\ 
    $\ds\expc\{X^2\} = \sum_{k = 0}^\infty k^2\frac{(\lambda\,T)^k}{k!}e^{-\lambda\,T} = e^{-\lambda\,T}\sum_{k = 1}^\infty k^2\frac{(\lambda\,T)^k}{k!} = e^{-\lambda\,T}\sum_{k = 1}^\infty k\frac{(\lambda\,T)^k}{(k - 1)!} = e^{-\lambda\,T}\sum_{k = 0}^\infty (k + 1)\frac{(\lambda\,T)^{k + 1}}{k!} \\= e^{-\lambda\,T}\sum_{k = 0}^\infty k\frac{(\lambda\,T)^{k + 1}}{k!} + e^{-\lambda\,T}\sum_{k = 0}^\infty\frac{(\lambda\,T)^{k + 1}}{k!} = e^{-\lambda\,T}\sum_{k = 1}^\infty\frac{(\lambda\,T)^{k + 1}}{(k - 1)!} + \lambda = e^{-\lambda\,T}\sum_{k = 0}^\infty\frac{(\lambda\,T)^{k + 2}}{k!} + \lambda = (\lambda\,T)^2 + \lambda\,T = m^2 + m$. 故 $\ds\var\{X\} = \expc\{X^2\} - (\expc\{X\})^2 = m^2 + m - m^2 = m$.
  \end{enumerate}
\end{sol}
...
\begin{sol}
  每小時 20 通, 則每分鐘平均 $\lambda = \frac{1}{3}$ 通; 在 5 分鐘時間區間中, 平均電話通數為 $m = \lambda\,T = \frac{1}{3}\times 5 = \frac{5}{3}$. 故 $\ds P(k)\equiv P(k,\,5) = \frac{(\frac{5}{3})^k}{k!} e^{-\frac{5}{3}},\quad k=0,\,1,\,2,\,\cdots$.  
  \begin{enumerate}[label=(\alph*)]\setlength{\itemsep}{0pt}
...
\begin{sol}
  Let $N$ be the number of claims filed; we have $\ds\prb\{N = 2\} = \frac{e^{-m}m^2}{2!} = 3\,\prb\{N = 4\} = \frac{e^{-m}m^4}{4!} \ie \frac{1}{2}m^2 = \frac{3}{24}m^4 \ie m^2 = 4 \ie m = 2$. 
\end{sol}
...
\begin{sol}
  ...
  \begin{align*}
    ...
    &\quad = \sum_{k = 0}^\infty 0.75^k\,\frac{e^{-5}{5^k}}{k!} = e^{-5}\sum_{k = 0}^\infty \frac{(0.75\cdot 5)^k}{k!} = e^{-5}\,e^{0.75\cdot 5} \approx 0.287
  \end{align*}
\end{sol}
...
\begin{sol}
  Let $X$ be the number of tornadoes and $Y$ be the conditional distribution of $X$ given that $X > 0$. Then $\ds p(y)\equiv\prb\{Y = y\} = \prb\{X = y\,|\,X > 0\} = \frac{\prb\{X = y\}}{\prb\{X > 0\}} = \frac{1}{1 - e^{-3}}\,\frac{e^{-3}\,3^y}{y!},\;y = 1,\,2,\,\ldots$. Note that $Y$ is $\ds\frac{1}{1 - e^{-3}}$ times a Poisson distribution with mean $3$, so $\ds\expc\{Y\} = \frac{3}{1 - e^{-3}}$, $\ds\expc\{Y^2\} = \frac{3 + 3^2}{1 - e^{-3}} = \frac{12}{1 - e^{-3}}$, $\ds\var\{Y\} = \expc\{Y^2\} - (\expc\{Y\})^2 = \frac{12}{1 - e^{-3}} - \frac{3^2}{(1 - e^{-3})^2}\approx 2.6609$.
\end{sol}
