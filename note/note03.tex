...
\begin{sol}
  考慮 $\ds\sqrt{1 + t}$ 在 $\ds[0, x]$, $\ds x > 0$. 由 MVT $\ds\exists\,c\in(0, x)\;\sqrt{1 + x} - 1 = \frac{1}{2\sqrt{1 + c}} x < \frac{1}{2}x \ie \sqrt{1 + x} < 1 + \frac{1}{2}x$. 
\end{sol}
...
\begin{sol}
  令 $\ds f(x) = \sin^{-1}\frac{x - 1}{x + 1} - 2\tan^{-1}\sqrt{x} + \frac{\pi}{2}$. $\forall\,x > 0$, $\ds f'(x) = \frac{1}{\sqrt{1 - (\frac{x - 1}{x + 1})^2}}\cdot\frac{(x + 1) - (x - 1)}{(x + 1)^2} - 2\frac{1}{1 + x}\cdot\frac{1}{2\sqrt{x}} = 0$ $\ie$ $f(x)$ 為常數 $\ie$ $\ds f(x) = f(1) = -2\cdot\frac{\pi}{4} + \frac{\pi}{2} = 0$; 由 $f(x)$ 之連續性或代入 $x = 0$ 均可得 $f(0) = 0$.   
\end{sol}
