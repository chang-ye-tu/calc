...
\begin{ex}[LHR 與 DNE]
  若 $\ds\lim_{x\to a}f(x) = 0$，$\ds\lim_{x\to a}g(x) = 0$ 而 $\ds\lim_{x\to a}\frac{f'(x)}{g'(x)} = \text{DNE}$，$\ds\lim_{x\to a}\frac{f(x)}{g(x)}$ 仍存在。取 $\ds f(x) = x^2\sin\frac{1}{x}$，$g(x)= x$ 與 $a = 0$，則 $\ds\lim_{x\to 0} f(x) = 0$ 且 $\ds\lim_{x\to 0} g(x) = 0$，$\ds\lim_{x\to 0}\frac{f'(x)}{g'(x)} = \lim_{x\to 0}\left(2x\sin\frac{1}{x} -\cos\frac{1}{x}\right) = \text{DNE}$，但 $\ds\lim_{x\to 0}\frac{f(x)}{g(x)} = \lim_{x\to 0}\frac{x^2\sin\frac{1}{x}}{x} = \lim_{x\to 0} x\sin\frac{1}{x}= 0$。
\end{ex}
